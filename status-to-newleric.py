...
def send_metric(name: str, value: str):
    """
    NewRelicへSwitchbotデータを送信する

    Args:
        name (str): Metricとして登録する名前
        value (str): Switchbotで取得したデータ値
    """    
    headers = {
        "Content-Type": "application/json",
        "Api-Key": newleric_licensekey,
    }
...
if __name__ == "__main__":
    ...
    newleric_licensekey = get_env_variable("NEWLERIC_LICENSEKEY")
    ...
    try:
        ...
        # 温度と湿度を送信
        send_metric("temperature", devices["body"]["temperature"])
        send_metric("humidity", devices["body"]["humidity"])
